...
rem  the root's travel distance (m), speed (m/s), net turn (deg), turn rate
rem  (deg/s) -- and gait_mps, the stance foot's speed under the root, which is
rem  the authored travel speed of the in-place cycles (walk/run/dash loops).
rem  These are the real numbers MGSV's movement was authored with -- measured
rem  off the animation data, not guessed -- for a 1:1 movement rebuild.
rem  Finishes by grouping everything into rips\locomotion-params\cruise-table.tsv.
rem
...
echo [1/5] TPP player (male base) locomotion...
...
echo [2/5] TPP player (female base) locomotion...
...
echo [5/5] Cruise-speed table from the measured params...
"%TOOL%" --analyze-locomotion "%OUT%" --out "%OUT%" >> "%LOGS%\95-measure.log" 2>&1
if errorlevel 1 set /a ERRORS+=1

echo finished, %ERRORS% problem step(s) >> "%LOGS%\95-measure.log"
echo.
echo done - %ERRORS% problem step(s).
echo   per-clip tables:  %OUT%\*\locomotion-params.tsv
echo   cruise table:     %OUT%\cruise-table.tsv
