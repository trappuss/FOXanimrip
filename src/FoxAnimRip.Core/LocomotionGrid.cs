...
{
    // dh is dash: verified present in the player sets (snapnon_s_dh_st_l0_l and
    // family) alongside wk and rn, same phase and tail grammar.
    private static readonly Regex Pattern = new(
        @"^(?<family>.+?)_(?<verb>wk|rn|jg|dh)_(?<phase>st|lp|tn|ed)_(?<tail>.+)$",
