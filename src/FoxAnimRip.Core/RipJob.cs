...
public static class RipJob
{
                                ...
                                CancellationToken token = default)
    {
        ...
        var (footL, footR) = o.Measure ? FindFeet(model) : (-1, -1);
        if (o.Measure && footL < 0 && footR < 0)
            log("! no *LFOOT/*RFOOT bones resolved on this model; gait_mps will "
                + "be 0 (a non-biped rig, or the name dictionary is not loaded)");
            ...
            for (var si = 0; si < input.Sources.Count; si++)
            {
                ...
                foreach (var entry in set.Entries)
                {
                    ...
                    if (o.Measure)
                    {
                        ...
                        // in-place one would report zero for everything. World
                        // positions are captured alongside for the stance-foot
                        // measurement, which recovers the authored speed of the
                        // cycles whose root does NOT travel (see GaitSpeed).
                        ExportClip mclip;
                        var frames = RootBake.Frames(anim, o.Step);
                        var wpos = new Vector3[model.Bones.Count][];
                        for (var b = 0; b < wpos.Length; b++)
                            wpos[b] = new Vector3[frames];
                        try
                        {
                        ...
                        var (gait, stance) = RootBake.GaitSpeed(wpos, footL, footR,
                                                                mclip.Fps);
                        measureRows.Add(
                            $"{stem}\t{clipName}\t{mclip.FrameCount}\t{mclip.Fps:0.###}\t"
                            + $"{dist:0.####}\t{speed:0.####}\t{yaw:0.##}\t{turn:0.##}\t"
                            + $"{match}\t{gait:0.####}\t{stance}");
                        result.Exported++;
                        inThisMtar++;
                        continue;
        ...
        if (o.Measure)
        {
            ...
            var header = "mtar\tclip\tframes\tfps\tdistance_m\tspeed_mps\t"
                       + "netYaw_deg\tturnRate_dps\tmatchedBones\tgait_mps\t"
                       + "gaitSamples";
    ...
    /// <summary>
    /// The foot bones, found by their dictionary names.
    ///
    /// Every biped Fox skeleton in these games names them <c>SKL_032_LFOOT</c>
    /// and <c>SKL_042_RFOOT</c> (with game-specific numbering), so matching on
    /// the suffix is exact, not fuzzy. Comes back (-1, -1) on rigs without them
    /// -- animals, props -- or when the name dictionary is not loaded, in which
    /// case the gait measurement reports 0 rather than guessing at bones.
    /// </summary>
    internal static (int Left, int Right) FindFeet(FmdlModel model)
    {
        int left = -1, right = -1;
        for (var i = 0; i < model.Bones.Count; i++)
        {
            var ni = model.Bones[i].NameIndex;
            if (ni < 0 || ni >= model.Names.Count) continue;
            var label = StrCodeNames.Label(model.Names[ni], null);
            if (label is null) continue;
            if (label.EndsWith("LFOOT", StringComparison.OrdinalIgnoreCase))
                left = i;
            else if (label.EndsWith("RFOOT", StringComparison.OrdinalIgnoreCase))
                right = i;
        }
        return (left, right);
    }
